...
export default function ProjectNavigationPane({ listofProjects, selectedProjectRef, setActivity, ...props }) {
  if (!listofProjects) {
    return null;
  }
  ...
  return (
    <nav className={`py-8 overflow-x-hidden max-h-[500px] ${classes.navpane}`}>
      {listofProjects.map((entry, index) => (
        <li onClick={handleSelectProject} key={`list:${index}`} id={index} className="text-white list-none cursor-pointer text-nowrap py-2">
          {entry.projectName}
        </li>
      ))}
    </nav>
